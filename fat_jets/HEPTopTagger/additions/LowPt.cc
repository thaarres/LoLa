...
const bool LowPt::is_tagged(PseudoJet htt_result) const {
  if (!htt_result.has_structure_of<HTT>()) return false;

  HTTStructure htt = htt_result.structure_of<HTT>();
  ...
  double FWM_W1W2_U1 = FWM(htt_result,11).U(1);
  double FWM_pW1W2_U1 = FWM(htt_result,1011).U(2);
  double FWM_pbW1W2_U1 = FWM(htt_result,1111).U(2);
  double FWM_pbW2_U1 = FWM(htt_result,1101).U(1);
